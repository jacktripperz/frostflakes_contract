...
contract FrostFlakes is Context {
    ...
    uint256 private WEEKLY_MAX_FROST_FLAKES = 604800; // 1 week
    uint256 private FROSTFLAKE_PER_BNB = 6048000000;
    uint256 private SECONDS_PER_DAY = 86400;
    uint256 private LEVEL_1_REWARD = 2;
    uint256 private LEVEL_2_REWARD = 2;
    uint256 private LEVEL_3_REWARD = 3;
    uint256 private LEVEL_4_REWARD = 4;
    uint256 private LEVEL_5_REWARD = 5;
    uint256 private REQUIRED_FREEZES_BEFORE_DEFROST = 6;
    uint256 private LIQUIDITY_FEE = 2;
    uint256 private CF = 2;
    uint256 private REF_BONUS = 5;
    uint256 private FIRST_DEPOSIT_REF_BONUS = 5; // 5 for this bonus + 5 on ref bonus = 10 total on first deposit
    uint256 private USE_OWNER_AS_REF_DEPOSITLINE_BONUS = 2;
    uint256 private DEPOSIT_BONUS = 2;
    ...
    uint256 private MIN_CAMPAIGN_DEPOSIT = 100000000000000000; // 0.1 BNB
    uint256 private MIN_CAMPAIGN_REF_COUNT_FOR_CLAIM = 3;
    ...
    function buyFrostFlakes(address ref) public payable {
        ...
        uint256 liquidityFee = calcPercentAmount(msg.value, LIQUIDITY_FEE);
        uint256 bnbValue = Math.sub(msg.value, liquidityFee);
        ...
        cfmAddress.transfer(liquidityFee);
    ...
    function airdrop(address reciever) payable external {
        ...
        airdrops_sent[msg.sender] = Math.add(airdrops_sent[msg.sender], Math.sub(msg.value, calcPercentAmount(msg.value, CF)));
        airdrops_sent_count[msg.sender] = Math.add(airdrops_sent_count[msg.sender], 1);
        airdrops_received[reciever] = Math.add(airdrops_received[reciever], Math.sub(msg.value, calcPercentAmount(msg.value, CF)));
        airdrops_received_count[reciever] = Math.add(airdrops_received_count[reciever], 1);
    ...
    function claimCampaignReward(uint256 campaignId) public payable {
        ...
        require(aHProtocolInitialized == false, "AH is active");
        require(campaignRunning == true, "Campaign needs to be stopped");
        ...
        for (uint i = 0; i < campaignReferrals[msg.sender].length; i++) {
            if (campaignReferrals[msg.sender][i].campaignId == campaignId) {
                campaignReferrals[msg.sender][i].claimed = true;
                canClaim = false;
            }
        }
    ...
    function hasClaimedCampaignReward(uint256 campaignId) public view returns(bool) {
        for (uint i = 0; i < campaignReferrals[msg.sender].length; i++) {
            if (campaignReferrals[msg.sender][i].campaignId == campaignId) {
                return campaignReferrals[msg.sender][i].claimed;
            }
        }
    ...
    function calcSellFrostFlakes(uint256 frostFlakes) public view returns(uint256) {
        uint256 bnbInWei = Math.mul(frostFlakes, FROSTFLAKE_PER_BNB);
        return bnbInWei;
    }

    function calcBuyFrostFlakes(uint256 bnbInWei) public view returns(uint256) {
        uint256 frostFlakes = Math.div(bnbInWei, FROSTFLAKE_PER_BNB);
        return frostFlakes;
    }
    ...
    function getLastDefrostOrPresaleEnded(address adr) public view returns(uint256) {
        uint256 lastFreezeOrPresaleEnded = lastDefrost[adr];
        if (presaleEndedAt > lastDefrost[adr]) {
            lastFreezeOrPresaleEnded = presaleEndedAt;
        }
        return lastFreezeOrPresaleEnded;
    }
    ...
    function getFrostFlakesSincelastFreeze(address adr) public view returns(uint256) {
        ...
        uint256 maxFrostFlakes = MAX_FROST_FLAKES;
        if (isOwner(adr)) {
            maxFrostFlakes = WEEKLY_MAX_FROST_FLAKES;
        }
        uint256 secondsPassed=Math.min(maxFrostFlakes,Math.sub(block.timestamp,lastFreezeOrPresaleEnded));
    ...
    function calcDailyRewardLevel(address adr) public view returns(uint256) {
        if (isOwner(adr)) {
            return LEVEL_1_REWARD;
        }

        uint256 lastDefrostOrPresaleEnded = getLastDefrostOrPresaleEnded(adr);

        if (lastDefrostOrPresaleEnded == 0) {
            return LEVEL_1_REWARD;
        }

        uint256 secondsPassedSinceLastDeFrost = Math.sub(block.timestamp,lastDefrostOrPresaleEnded);
    ...
    function calcFrostFlakesReward(uint256 secondsPassed, uint256 dailyReward, address adr) private view returns(uint256) {
        uint256 rewardsPerDay = calcPercentAmount(lockedFrostFlakes[adr], dailyReward); //  Math.div(Math.mul(29675377,2),100); = 593507
        uint256 rewardsPerSecond = Math.div(rewardsPerDay, SECONDS_PER_DAY); // 593507 / 86400 = 6
        uint256 frostFlakes = Math.mul(rewardsPerSecond, secondsPassed); // 6 * 108000 = 648000 
        return frostFlakes;
    }

    function calcFrostFlakesReward2(uint256 secondsPassed, uint256 dailyReward, address adr) private view returns(uint256) {
        uint256 rewardsPerDay = calcPercentAmount(Math.mul(lockedFrostFlakes[adr], 100), dailyReward); //  Math.div(Math.mul(Math.mul(29675377, 100),2),100); = 593507
        uint256 rewardsPerSecond = Math.div(rewardsPerDay, SECONDS_PER_DAY); // 59350754 / 86400 = 686
        uint256 frostFlakes = Math.mul(rewardsPerSecond, secondsPassed); // 686 * 108000 = 74188442
        // 74188442 / 100 = 741884 
        return frostFlakes;
    }
}
